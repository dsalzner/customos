...
unsafe fn shell_command() {
  if shell_check_command("help") {
    ...
    let (param, length) = shell_command_get_param("ed");
...
pub unsafe extern "C" fn shell_update() {
  if STATE == EState::SHELL {
	...
	shell_render_char(x, y, SHELL_TEXT_BUFFER[i], SHELL_TEXT_COLOR_BUFFER[i], COLOR_BLUE);
